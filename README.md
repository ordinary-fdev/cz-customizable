...
[![npm monthly downloads](https://img.shields.io/npm/dm/cz-customizable-2.svg?style=flat-square)](https://www.npmjs.com/package/cz-customizable-2)
[![bundlephobia](https://badgen.net/bundlephobia/minzip/cz-customizable-2)](https://bundlephobia.com/package/cz-customizable-2)


You have two ways to use `cz-customizable-2`. Originally, this project started as a commitizen plugin (Option 1). We introduced the second option to run this `cz-customizable-2` in standalone mode (Option 2), just like any NodeJS script. It's recommended to use `Option 2` for simplicity. The way you configure is shared between both options.
...
## cz-customizable-2 via global install

`npm i cz-customizable-2 -g`

Then run `cz-customizable-2` from your root repo. You can also use the alias `cz-cust`.

If everything is correct, you should see commit questions like the image above.

# Slow start

## Option 1 - cz-customizable-2 via npx

`npx cz-customizable-2`


## Option 2 - cz-customizable-2 in standalone mode

Use `cz-customizable-2` without `commitzen`.

* npm install `npm install cz-customizable-2 --save-dev`
* add a new script to your `package.json`:

```
"scripts" : {
  ...
  "commit": "./node_modules/cz-customizable-2/standalone.js"
}
```
...
## Option 3 - cz-customizable-2 as commitizen plugin
...
* configure `commitizen` to use `cz-customizable-2` as plugin. Add those lines to your `package.json`:

```
...
"config": {
  "commitizen": {
    "path": "node_modules/cz-customizable-2"
  }
}
```
...
* `cz-customizable-2` will first look for a file called `.cz-config.js` or `.config/cz-config.js` in the project root, near your `package.json`
...
This is suitable when your team is not ready to roll `cz-customizable-2` across all teams but you still would like to use it for your own commits, no matter the project.
...
* npm install -g cz-customizable-2. Make sure you have version `>v5.6.x`
* create global commitizen config file `.czrc`: `echo '{ "path": "cz-customizable-2" }' > ~/.czrc`
...
* **appendBranchNameToCommitMessage**: If you use `cz-customizable-2` with `cz-customizable-ghooks`, you can get the branch name automatically appended to the commit message. This is done by a commit hook on `cz-customizable-ghooks`. This option has been added on `cz-customizable-ghooks`, v1.3.0. Default value is `true`.
